...
class PayMemoRepository {
  ...
  //   await _firestore.collection('paymemo').add(paymemo.toDocument());
  // }
  // Future<void> createPayMemo(PayMemo paymemo) async {
  //   var docRef = await _firestore.collection('paymemo').add(paymemo.toDocument());
  //   print("PayMemo created with ID: ${docRef.id}");
  // }
  Future<void> createPayMemo(PayMemo paymemo) async {
  ...
  //           PayMemo.fromDocument(doc.data() as Map<String, dynamic>, doc.id)).toList());
  // }
  Stream<List<PayMemo>> readPayMemos() {
    return _firestore.collection('paymemo')
        .snapshots()
  ...
  //     });
  //     return snapshot.docs.map((doc) => PayMemo.fromDocument(doc.data() as Map<String, dynamic>, doc.id)).toList();
  //   });
  // }


  // Future<void> updatePayMemo(String paymemoId, PayMemo paymemo) async {
  //   await _firestore.collection('paymemo').doc(paymemoId).update(paymemo.toDocument());
  // }
  Future<void> updatePayMemo(String paymemoId, PayMemo paymemo) async {
  ...
  Future<void> deletePayMemo(String memoId) async {
    await _firestore.collection('paymemo').doc(
        memoId).update({'status': 'D'});
  }
}